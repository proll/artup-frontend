whp.DailyListMenuView = Backbone.View.extend({
	...
	render: function(){
		...
		this.$el.find("a").each(function() {
			$this = $(this);
			$this.attr("href", "/explore/daily/" + $this.data("id") + "/" + that.model.get("sort") + "/" + that.model.get("filter") + "/");
		});
	},
	...
	mabyNeedMore: function () {
		if(!this.model.get("loading") && !this.model.get("sleeped")) {
			if( this.$el.parent().scrollTop()+100 >= this.$el.height() - this.$el.parent().height() ) {
